                ...
                int nth)
{
    ...
    assert(!(lda % 16));
    assert(!(ldb % 16));
    tinyBLAS<16, f32x16, f32x16, float, float, float> tb{ A, lda, B,   ldb,
                                                          C, ldc, ith, nth };
#elif defined(__AVX__) || defined(__AVX2__)
    assert(!(lda % 8));
    assert(!(ldb % 8));
    tinyBLAS<8, f32x8, f32x8, float, float, float> tb{ A, lda, B,   ldb,
                                                       C, ldc, ith, nth };
#elif defined(__SSE__) || defined(__ARM_NEON)
    assert(!(lda % 4));
    assert(!(ldb % 4));
    tinyBLAS<4, f32x4, f32x4, float, float, float> tb{ A, lda, B,   ldb,
                                                       C, ldc, ith, nth };
#else
...
T*
ansi_malloc(int m, int n, U* out_lda)
{
    void* ptr;
    int b = 64 / sizeof(T);
    int lda = (n + b - 1) & -b;
...
template<typename T, typename U>
T*
ansi_new_test_matrix(int m, int n, U* out_lda)
{
