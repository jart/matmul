...
inline int bsr(unsigned long long x) {
    return __builtin_clzll(x) ^ 63;
}

inline unsigned long long roundup2pow(unsigned long long x) {
    return x > 1 ? 2ull << bsr(x - 1) : x ? 1 : 0;
}

inline unsigned long long morton(unsigned hi, unsigned lo) {
#ifdef __BMI2__
...
inline unsigned long long vorton(unsigned hi, unsigned lo) {
    unsigned stride = VECTOR_WIDTH / sizeof(float);
    unsigned vector = lo / stride;
    unsigned element = lo % stride;
    unsigned long long index = morton(vector, hi);
    return index * stride + element;
}
...
template <typename T> void randomize(int m, int n, T *A) {
    for (int j = 0; j < n; ++j)
        for (int i = 0; i < m; ++i) {
            A[vorton(j, i)] = numba();
        }
    ...
    template <int RM, int RN> void gemm(int m0, int m, int n0, int n, int k) {
        ...
        for (int job = start; job < end; ++job) {
            ...
            for (int l = 0; l < k; l += KN) {
                for (int j = 0; j < RN; ++j)
                    for (int i = 0; i < RM; ++i)
                        Cv[j][i] = madd(load<VECTOR>(A + vorton(ii + i, l)),
                                        load<VECTOR>(B + vorton(jj + j, l)), Cv[j][i]);
            }
            TC Cd[RN][RM];
            for (int j = 0; j < RN; ++j)
                for (int i = 0; i < RM; ++i)
...
template <typename T> T *llamafile_malloc(int m, int n) {
    void *ptr;
    size_t size = sizeof(T) * (vorton(n, m) + 1);
